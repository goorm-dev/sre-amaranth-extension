...
echo "▸ 서빙본 대조"
# DNS 가 아직 안 붙었을 수 있으니 ALB 에 Host 헤더로 직접 물어본다.
HOST=worktime.goorm.io
ALB=$(kubectl --context "$CTX" -n worktime get ingress worktime-web \
...
for f in app.js index.html app.css lib/auth.js lib/gwapi.js lib/leave.js; do
  ...
  for i in 1 2 3 4 5 6; do
    S=$(curl -sk "https://$ALB/$f?$(date +%s)-$i" -H "Host: $HOST" | shasum -a256 | cut -c1-16)
    [ "$L" = "$S" ] && { ok=1; break; }
    sleep 3
  done
  [ -n "$ok" ] && echo "  ✓ $f" || { echo "  ✗ $f  로컬=$L 서빙=$S"; exit 1; }
done
echo "완료: https://$HOST  (ALB $ALB)"
