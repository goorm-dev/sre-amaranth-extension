...
(function () {
  ...
  // 대부분 하루짜리다. 시작을 고르면 종료도 같이 맞춰 둔다.
  for (const [a, b] of [['lvDate', 'lvEnd'], ['brDate', 'brEnd']]) {
  ...
  async function lvPreview() {
    ...
    const ek = $('lvEnd').value || dk;
    if (!dk) { $('lvMsg').textContent = '날짜를 선택해 주세요.'; return; }
    if (ek < dk) { $('lvMsg').textContent = '종료 날짜가 시작보다 앞섭니다.'; return; }
    $('lvNext').disabled = true; $('lvMsg').textContent = '확인 중…';
    try {
      const pv = await GW.leave.preview($('lvType').value, dk, ek, { startTm: $('lvStart').value });
    ...
    try {
      ...
      $('brPreview').innerHTML =
        `<div class="r"><span>날짜</span><b>${pv.startKey === pv.endKey
          ? esc(T.label(T.fromKey(pv.startKey)))
          : `${pv.startKey} ~ ${pv.endKey} (${Number(pv.appDy)}일)`}</b></div>`
        + `<div class="r"><span>시간</span><b>${lvHm(pv.span.start)} ~ ${lvHm(pv.span.end)}`
          + `${pv.startKey === pv.endKey ? '' : ' (매일)'}</b></div>`
        + `<div class="r"><span>인정 시간</span><b>${T.fmtDuration(pv.appTm)}</b></div>`
  ...
  async function tmShow() {
    ...
    try {
      const merged = await GW.team.syncRooms(await tmSelf(frCfg), frCfg);
      if (merged !== frCfg) {
